...
func (p *TextParser) initPatterns() {
	p.patterns = []patternEntry{
		...
		// handleACK can prefer it over the subkind when computing terminal status.
		{
			name: "ack",
			regex: regexp.MustCompile(
				`(?i)^(?P<id>[A-Za-z0-9_.:-]+):\s*(?P<kind>(?:SCAN|DEVICE_SCAN|DRONE|DEAUTH|RANDOMIZATION|BASELINE|CONFIG|TRIANGULATE(?:_STOP)?|TRI_START|STOP|START|REBOOT|BATTERY_SAVER(?:_START|_STOP)?|VIBRATION_(?:ON|OFF)|HB|HITS_RESET|DEBOUNCE|CODE|PROBE|RAW_BLE|MODEL_SET|DEDUP|CONF|WIFI_(?:ON|OFF|SSID|PSK)|TARGET_INTERVAL)_ACK):?(?P<status>[A-Z0-9_]*)(?::(?P<value>[A-Z0-9_,.\- +]+))?`),
			handler: p.handleACK,
		},
